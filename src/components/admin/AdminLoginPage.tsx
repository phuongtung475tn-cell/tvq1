...
export function AdminLoginPage() {
  ...
  async function handleSubmit() {
    ...
    else setError("Đăng nhập thất bại. Kiểm tra Supabase Auth và app_metadata.role = admin.");
  }
  ...
  return <main className="flex min-h-screen items-center justify-center bg-neutral-950 px-4 text-white"><div className="w-full max-w-sm rounded-2xl bg-neutral-900 p-6 ring-1 ring-white/10">
    ...
    {authed ? <div className="space-y-3 text-center"><p className="text-sm text-emerald-400">Đã đăng nhập.</p><a href="/" className="block w-full rounded-lg bg-white py-2.5 text-sm font-bold text-neutral-900">Vào trang quản trị</a></div> : <div className="space-y-3">
      <input ref={passwordInputRef} type="password" value={password} onChange={(e) => { setPassword(e.target.value); setError(""); }} placeholder="Mật khẩu Supabase Auth" autoComplete="current-password" autoFocus onKeyDown={(e) => { if (e.key === "Enter") void handleSubmit(); }} className="w-full rounded-lg bg-neutral-800 px-3 py-2.5 text-sm outline-none ring-1 ring-white/10" />
      {error && <p className="text-xs text-red-400">{error}</p>}<button type="button" onClick={() => void handleSubmit()} disabled={!configReady || !email || !supabaseUrl || !supabaseKey} className="w-full rounded-lg bg-white py-2.5 text-sm font-bold text-neutral-900 disabled:opacity-60">{configReady ? "Đăng nhập" : "Đang tải cấu hình..."}</button>
    </div>}
  </div></main>;
}
